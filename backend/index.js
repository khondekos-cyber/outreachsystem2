...
// --- 1. START SERVER IMMEDIATELY (Prevents Render Timeout) ---
const app = express();
...
app.get('/', (req, res) => {
    if (latestQr) {
        ...
        res.send('<h1>WhatsApp Engine Starting...</h1><p>Please wait 30 seconds and refresh.</p><script>setTimeout(() => location.reload(), 5000);</script>');
    }
});
...
client.on('qr', (qr) => {
    latestQr = qr; // Update the web page QR
    qrcode.generate(qr, { small: true });
    console.log('👉 NEW QR CODE GENERATED. Check your Render URL to scan.');
});
...
async function orchestrate(msg, isOutreach) {
    ...
    if (msg.timestamp < (Date.now() / 1000) - 60) return; // Ignore old messages
